...
def get_features(video_data, frame_batch_size=64, device='cuda'):
    ...
    with torch.no_grad():
        ...
        features_mean, features_std = extractor(last_batch)  # ([1, 2048, 1, 1])
        output1 = torch.cat((output1, features_mean), 0)  # ([len, 2048, 1, 1])
        output2 = torch.cat((output2, features_std), 0)
        output = torch.cat((output1, output2), 1).squeeze()  # ([len, 2048 * 2, 1, 1])  --> # ([len, 2048 * 2])

    return output  # [len , 2048 * 2]
